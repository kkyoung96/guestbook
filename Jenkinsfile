...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="kkyoung96/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/kkyoung96/guestbook.git'
            }
        }
        stage('Build') {
            steps {
        ...
        stage ('JMeter LoadTest') {
            steps { 
                echo "sh '~/lab/sw/jmeter/bin/jmeter.sh -j jmeter.save.saveservice.output_format=xml -n -t src/main/jmx/guestbook_loadtest.jmx -l loadtest_result.jtl' "
    ...
    post { 
        always { 
            emailext (attachLog: true, body: '본문', compressLog: true
                    , recipientProviders: [buildUser()], subject: '제목', to: 'kkyoung96.j@gmail.com')
